...
#define PROFILER_LOG(fmt, ...) do {                                  \
    ...
    profiler_log_fn("[idk_profiler] " fmt "\n", ##__VA_ARGS__);       \
} while (0)
...
static int idk_profiler_claim_path(char *path, size_t path_sz) {
    pid_t my_pid = getpid();
    char pid_str[16];
...
__attribute__((constructor)) static void idk_profiler_init() {
    char path[4096];
    if (idk_profiler_claim_path(path, sizeof(path)) < 0) return;
