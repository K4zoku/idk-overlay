...
int syringe_inject_dotnet(pid_t pid, const char *so_path) {
  ...
  if (env && env[0]) {
    ...
    if (slash) {
      ...
      snprintf(profiler_path, sizeof(profiler_path), "%.*s/libidk-dotnet-profiler.so", (int)dir_len, abs_path);
    } else {
      snprintf(profiler_path, sizeof(profiler_path), "./libidk-dotnet-profiler.so");
    }

    if (access(profiler_path, F_OK) != 0) {
      snprintf(profiler_path, sizeof(profiler_path), "/usr/local/lib/libidk-dotnet-profiler.so");
      if (access(profiler_path, F_OK) != 0) {
        snprintf(profiler_path, sizeof(profiler_path), "/usr/lib/libidk-dotnet-profiler.so");
      }
    }
  }
