 ...
 * RTLD_NOLOAD first - reuse an already-loaded library: the target's GL
 * context is bound to the specific libGL.so.1 it loaded, so we must use
 * the SAME library. */
...
#undef GL_RESOLVE
  ...
  if (!idk_fn_glGetIntegerv || !idk_fn_glDrawArrays || !idk_fn_glUseProgram) {
    IDK_ERR("gl-loader",
            "Critical GL functions missing - "
            "glGetIntegerv=%p glDrawArrays=%p glUseProgram=%p\n",
            (void *)idk_fn_glGetIntegerv, (void *)idk_fn_glDrawArrays, (void *)idk_fn_glUseProgram);
    return -1;
  }
