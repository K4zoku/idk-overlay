...
static GLuint gl_dmabuf_to_texture(int dmabuf_fd, uint32_t w, uint32_t h, uint32_t stride, uint32_t fourcc) {
...
static GLuint egl_dmabuf_to_texture(int dmabuf_fd, uint32_t w, uint32_t h, uint32_t stride, uint32_t format,
...
static GLuint shm_to_texture(int shm_fd, uint32_t w, uint32_t h, uint32_t pixel_size, uint32_t buffer_idx,
                             int premultiplied) {
...
GLuint egl_dmabuf_to_texture(int dmabuf_fd, uint32_t w, uint32_t h, uint32_t stride, uint32_t format, uint64_t modifier,
  ...
  if (drm_fmt == 0)
    drm_fmt = 0x34324742; /* fallback: BGRA8888 */
  ...
  /* Resolve EGL image binding functions from the already-loaded GL lib
   * (no dlopen here — the render path must never dlopen in wine).
   * idk_compositor_egl_init_gl resolves these from the game's libGL;
   * fall back to eglGetProcAddress if still missing. */
...
#undef GLCHECK
  ...
  glColorMask((GLboolean)last_color_mask[0], (GLboolean)last_color_mask[1], (GLboolean)last_color_mask[2],
              (GLboolean)last_color_mask[3]);
}
