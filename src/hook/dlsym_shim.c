 ...
 *
 * Wine's opengl32 resolves GLX/EGL entry points via handle-specific
 * dlsym() calls: dlsym(glhandle, "glXGetProcAddressARB") where glhandle
 * is wine's own dlopen of libGL.so.1. Those lookups never see plain
 * LD_PRELOAD exports, so intercepting dlsym() itself is the only way to
 * route the game's GL calls through our hooks. */
...
/* Resolve real libc functions. dlvsym takes glibc's loader lock — calling
 * lazily from a hook deadlocks wine (loader_section vs glibc lock), so
 * resolve from the constructor before wine loads DLLs. */
