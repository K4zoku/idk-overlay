...
EGLBoolean eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
  ...
  if (!fn_eglQuerySurface)
    fn_eglQuerySurface =
        (EGLBoolean (*)(EGLDisplay, EGLSurface, EGLint, EGLint *))real_dlsym(RTLD_DEFAULT, "eglQuerySurface");
  if (fn_eglQuerySurface && dpy && surface) {
    EGLint surf_w = 0, surf_h = 0;
    if (fn_eglQuerySurface(dpy, surface, EGL_WIDTH, &surf_w) && fn_eglQuerySurface(dpy, surface, EGL_HEIGHT, &surf_h))
...
static int install_egl_hook(void) {
  ...
  if (lib)
    egl_swap_addr = real_dlsym(lib, "eglSwapBuffers");
  ...
  if (egl_swap_addr) {
    int n = syringe_hook_install_addr("eglSwapBuffers", egl_swap_addr, (void *)eglSwapBuffers,
  ...
  int n = syringe_hook_install("eglSwapBuffers", (void *)eglSwapBuffers, (void **)&orig_eglSwapBuffers);
  if (n > 0) {
    g_hook_installed = 1;
...
int idk_egl_init(void) {
  if (g_hook_installed)
    return 0;
  /* EGL display init must happen here (hook install thread, before the
   * game renders) — doing it from the swap hook deadlocks wine. */
  idk_compositor_egl_preload();
