...
static VKAPI_ATTR VkResult VKAPI_CALL idk_CreateInstance(const VkInstanceCreateInfo *pCreateInfo,
                                                         const VkAllocationCallbacks *pAllocator,
                                                         VkInstance *pInstance) {
  VkLayerInstanceCreateInfo *chain_info = (VkLayerInstanceCreateInfo *)get_chain_info(
   ...
   *
   * Broker bootstrap lives here (not a constructor): the loader dlopens
   * this library into every Vulkan app, so nothing may run before the
   * game actually creates an instance. */
...
static VKAPI_ATTR void VKAPI_CALL idk_DestroyInstance(VkInstance instance, const VkAllocationCallbacks *pAllocator) {
  pthread_mutex_lock(&g_dispatch_lock);
...
static VKAPI_ATTR VkResult VKAPI_CALL idk_CreateDevice(VkPhysicalDevice physicalDevice,
                                                       const VkDeviceCreateInfo *pCreateInfo,
                                                       const VkAllocationCallbacks *pAllocator, VkDevice *pDevice) {
  VkLayerDeviceCreateInfo *chain_info = (VkLayerDeviceCreateInfo *)get_chain_info(
...
static VKAPI_ATTR void VKAPI_CALL idk_DestroyDevice(VkDevice device, const VkAllocationCallbacks *pAllocator) {
  pthread_mutex_lock(&g_dispatch_lock);
...
static VKAPI_ATTR VkResult VKAPI_CALL idk_CreateSwapchainKHR(VkDevice device,
...
static VKAPI_ATTR void VKAPI_CALL idk_DestroySwapchainKHR(VkDevice device, VkSwapchainKHR swapchain,
                                                          const VkAllocationCallbacks *pAllocator) {
  pthread_mutex_lock(&g_dispatch_lock);
  struct device_dispatch *dd = find_device(device);
...
static VKAPI_ATTR void VKAPI_CALL idk_GetDeviceQueue(VkDevice device, uint32_t queueFamilyIndex, uint32_t queueIndex,
                                                     VkQueue *pQueue) {
  pthread_mutex_lock(&g_dispatch_lock);
  struct device_dispatch *dd = find_device(device);
...
static VKAPI_ATTR VkResult VKAPI_CALL idk_QueuePresentKHR(VkQueue queue, const VkPresentInfoKHR *pPresentInfo) {
...
static VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL idk_GetDeviceProcAddr(VkDevice device, const char *pName);

static VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL idk_GetInstanceProcAddr(VkInstance instance, const char *pName) {
