...
static uint32_t keycode_to_scancode(unsigned int keycode) { return (keycode >= 8) ? (keycode - 8) : keycode; }
...
int x11_handle_key_event(XEventStorage *ev) {
  ...
  if (g_hotkey_keysym || g_hotkey_scancode) {
    ...
    if (g_hotkey_keysym && keysym == g_hotkey_keysym)
      m = 1;
    if (g_hotkey_scancode && scancode == g_hotkey_scancode)
      m = 1;
    if (m && (!g_hotkey_mods || (g_mods & g_hotkey_mods) == g_hotkey_mods))
      cap_match = 1;
  }
  ...
  if (!same_key && (g_hotkey_overlay_keysym || g_hotkey_overlay_scancode)) {
    ...
    if (g_hotkey_overlay_keysym && keysym == g_hotkey_overlay_keysym)
      m = 1;
    if (g_hotkey_overlay_scancode && scancode == g_hotkey_overlay_scancode)
      m = 1;
    if (m && (!g_hotkey_overlay_mods || (g_mods & g_hotkey_overlay_mods) == g_hotkey_overlay_mods))
      ovl_match = 1;
  }
  ...
  if (!g_webview_dead) {
    if (cap_match) {
      if (pressed && !g_hotkey_pressed) {
        ...
        if (same_key) {
          if (!g_captured) {
            idk_x11_input_set_capture(1);
            g_overlay_visible = 1;
            send_overlay_state(1);
          } else {
            idk_x11_input_set_capture(0);
          }
        } else {
