...
static int lib_loaded(const char *name) {
...
static void *hook_install_thread(void *arg) {
  ...
  if (g_hooks_installed)
    done[1] = 1; // glx already installed early
  for (int i = 0; i < 150; i++) {
...
static int idk_is_target_process(void) {
  ...
  if (cmd_fd >= 0) {
    ...
    if (cn > 0) {
      ...
      if (strstr(lower, "wineboot") || strstr(lower, "wineserver") || strstr(lower, "services.exe") ||
          strstr(lower, "explorer.exe") || strstr(lower, "svchost.exe") || strstr(lower, "winedevice.exe") ||
          strstr(lower, "mscorsvw.exe") || strstr(lower, "plugplay.exe") || strstr(lower, "rpcss.exe") ||
          strstr(lower, "msiexec.exe") || strstr(lower, "rundll32.exe") || strstr(lower, "cmd.exe") ||
          strstr(lower, "reg.exe") || strstr(lower, "schtasks.exe") || strstr(lower, "start.exe") ||
          strstr(lower, "bridge.exe") || strstr(lower, "_v2-entry-point")) {
        return 0;
      }
    }
  }
  ...
  static const char *blacklist[] = {"awk",  "gawk", "mawk", "sh",    "bash",     "dash",    "UpdateNix", "mktemp",
                                    "cp",   "mv",   "rm",   "chmod", "readlink", "dirname", "basename",  "head",
                                    "tail", "cut",  "grep", "sed",   "true",     "false",   NULL};
