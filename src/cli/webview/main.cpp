...
int main(int argc, char *argv[]) {
  ...
  QCommandLineOption socketOption({"s", "socket"}, "Socket path (overrides env/config).", "path");

  QCommandLineOption noDmaBufOption("no-dmabuf", "Force SHM mode (disable DMABUF zero-copy).");

  QCommandLineOption urlOption("url", "Override URL (bypasses config).", "url");

  QCommandLineOption widthOption("width", "Override initial width (bypasses config).", "pixels");

  QCommandLineOption heightOption("height", "Override initial height (bypasses config).", "pixels");
  ...
  Manager manager(configPath, socketPath, noDmaBuf, url, width, height, match);
