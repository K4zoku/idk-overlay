...
static inline void idk_timestamp(char *buf, size_t len) {
  ...
  snprintf(buf, len, "%02d:%02d:%02d.%03d", tm.tm_hour, tm.tm_min, tm.tm_sec, (int)(ts.tv_nsec / 1000000));
}
 ...
 * Includes millisecond timestamp in the prefix. */
#define IDK_LOG(tag, fmt, ...)                                                                                         \
  do {                                                                                                                 \
